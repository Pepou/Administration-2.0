...
from .Ui_Interface_Centrales_Annule_Remplace import Ui_Exploitation_Centrales_Annule_Remplace


class Exploitation_Centrales_Annule_Remplace(QMainWindow, Ui_Exploitation_Centrales_Annule_Remplace):
    """
...
#                print(date_fr)
    ...
    #        print("replace {}".format(self.copy_data))
            ...
            nbr_sondes = self.copy_data.shape[0] -1 ###equivalent à len(self.copy_data.columns)
...
#            pass
    ...
    def on_actionSauvegarder_triggered(self):
        ...
        if self.tupple_index:
            ...
            res = QMessageBox.question(
                self,
                self.trUtf8("Insertion BDD"),
                self.trUtf8("""Voulez vous Enregistrer l'annule et remplace"""),
                QMessageBox.StandardButtons(
                    QMessageBox.No |
                    QMessageBox.Yes))
    ...
    #            print(sauvegarde)
        #############test a faire voir sil ne manque paq des donnees
                ...
                sauvegarde["administratif"]["num_rapport"]= carto_bdd.insertion_carto_annule_remplace(sauvegarde)
#                print(sauvegarde["administratif"]["num_rapport"])
            
                if not(issubclass(type(sauvegarde["administratif"]["num_rapport"]), BaseException)): #test savoir si tout c'est bien passée
                    QMessageBox.information(
                    self,
                    self.trUtf8("Numero Cartographie"),
                    self.trUtf8(f"""Le numero de la cartographie est {sauvegarde["administratif"]["num_rapport"]}"""))
                    
    #                if sauvegarde["administratif"]["num_rapport"]:
                    res = QMessageBox.question(
                    self,
                    self.trUtf8("Rapport"),
    ...
    #                    file = QFileDialog.getSaveFileName(None ,  "Selectionner le dossier de sauvegarde de l'annexe", "y:/1.METROLOGIE/0.ARCHIVES ETALONNAGE-VERIFICATIONS/3-CARTOS-SIMULATION", '*.pdf' )
                    ...
                    self,
                    self.trUtf8("Enregistrement Annule et remplace Cartographie"),
